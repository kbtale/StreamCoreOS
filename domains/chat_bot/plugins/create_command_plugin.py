    ...
    async def execute(self, data: dict, context=None):
        try:
            ...
            cmd_id = await self.db.execute(
                "INSERT INTO chat_commands (name, response, cooldown_s) VALUES ($1, $2, $3) RETURNING id",
                [req.name, req.response, req.cooldown_s],
            )
            return {"success": True, "data": {
                "id": cmd_id, "name": req.name,
                "response": req.response, "cooldown_s": req.cooldown_s, "enabled": True,
            }}
